...
exports.findRoom = function (req,res) {
    ...
    var ref = db.ref("games/"+req.game._id);
    var refUser = db.ref("users/" + req.user._id);
    console.log(req.user._id);
    refUser.once("value",function(data){
        if(data.val().status==2){
            ref.child(data.val().room).set({"everything" : null},function(err){
                console.log(err);
            });
        }
        ref.child("max").once("value", function(snap) {
            if(snap.val()==0){
                key = ref.push().key;
                ref.child(key).child(req.user._id).set({"score" : 0},function(err){
                    if(!err)  {
                        ref.child(key).update({"people" : 1,"turn": "wait"});
                        ref.update({"max": req.game.room - 1});
                        refUser.update({ "status" : 2});
                        refUser.update({ "room" : key});
                        res.json({data : key,ready: false});
                    }
                });

            } else {
                ref.orderByValue().limitToLast(1).once("value",function (snap) {
                    if(!snap.val() || Object.keys(snap.val())[0] == 'max'){
                        ...
                        ref.child(key).child(req.user._id).set({"score" : 0},function(err){
                            if(!err)  {
                                people = snap.val()[key].people + 1;
                                max = req.game.room - people;
                                ref.child(key).update({"people" : people});
                                ref.update({"max": max});
                                refUser.update({ "status" : 2});
                                refUser.update({ "room" : key});
                                if(max == 0){
                                    res.json({data : key,ready: true});
                                } else {
                                    res.json({data : key,ready: false});
                                }
