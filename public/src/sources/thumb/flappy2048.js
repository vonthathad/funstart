...
document.getElementById('lgd').appendChild(game.div);

var deathflash = document.createElement('div');
deathflash.style.background = '#FF7631';
...
var ww = $('.game-area').width();
var hh = $('.game-area').height();
if(hh > ww) hh = ww;
var cell_size = parseInt( 88);
...
var flap = loadGameSound('/sources/game/20/flap.wav', 3);

var coin = loadGameSound('/sources/game/20/coin.wav', 1);

var punch = loadGameSound('/sources/game/20/punch.wav', 1);

game.bg = document.createElement('div');
game.bg.style.width = ww+'px';
game.bg.style.height = hh+'px';
game.bg.style.background = '#3d2963';
...
wing.img = loadGameImage('/sources/game/20/wing.png'); // 150 x 131
...
wingfill.img = loadGameImage('/sources/game/20/wingfill.png'); // 150 x 131
...
logo.showing = true;
logo.hiding = false;
logo.img = loadGameImage('/sources/game/20/logo.png'); // 626 x 188
...
gameover.showing = false;
gameover.hiding = true;
gameover.img = loadGameImage('/sources/game/20/gameover.png'); // 626 x 144
...
gameover.reposition = function () {
 ...
 gameover.img.style.top = Math.floor(Math.cos(gameover.fr/32)*0 + (hh - 200 - 144 - hh/9)/2)+'px';
};
...
sndo.div.style.backgroundImage = 'url("/sources/game/20/soundoff.png")';
sndo.has_sound = true;
...
if (hh > ww) {
 parody.div.innerHTML = 'Xoay ngang điện thoại rồi refresh để có trải nghiệm tốt hơn';//This website is a parody and is not affiliated with Flappy Bird or .GEARS Studios';
} else {
 parody.div.innerHTML = '';
}
parody.div.style.textAlign = 'center';
parody.div.style.padding = '20px';
...
parody.div.style.left = '0';//Math.round((ww - 500)/2)+'px';
parody.div.style.top = '0';
parody.div.style.right = '0';
parody.div.style.bottom = '0';
...
function newCell () {
 ...
 div.style.textAlign = 'center';
 div.style.fontFamily = '"Clear Sans", "Helvetica Neue", Arial, sans-serif';
 div.style.fontSize = ww/10 + 'px';
 div.style.color = '#606060';
 div.style.borderRadius = '10px';
 div.style.fontWeight = 'bold';
 div.style.marginTop = 'auto';
 div.style.marginBottom = 'auto';
 div.style.border = '4px solid #181027';
 ...
 cell.setValue = function (n) {
  ...
  if (n == 0) {
   ...
   this.vs = s;
   div.style.fontSize = Math.round(40 * Math.pow(9/(9 + Math.min(7, s.length)-1), 2.3))+'px';
  ...
  while (ltm < ftm) {
   ...
   if (game.started) {
    ...
    if (game.ended) { // •••• you have to implement this. when the game is over, this is set to true.
     ...
     setTimeout(function () {
      fsGame.gameOver({score: points*1000});
      var des;
      if(points<7){
       des = "Chia buồn chia buồn, chim của thí chủ bay quá yếu nên đã tử nạn rồi"
      } else if (points<14) {
       des = "Con chim này quả nhiên được chăm sóc cẩn thận, nếu như cố gắng thêm xíu nữa sẽ thành thiên hạ đệ nhất chim"
      } else {
       des = "Bái phục bái phục! Quả nhiên danh xưng thiên hạ đệ nhất chim là thuộc về thí chủ"
      }
      var obj = {
       title: 'Điểm: ' + points,
       des: des,
      ...
      fsGame.createShare({
       name: fsGame.user.displayName + " đã đạt " + points + " điểm.",
       des: des
      });
     },1000);
     if (points > highscore) {
...
$(document).ready(function(){
   fsGame.gameStart(function(){
    ...
    document.getElementById('lgd').onmousedown = function (e) {
     doFlap();
     e.preventDefault();
    }

    document.getElementById('lgd').addEventListener('touchstart', function(e){
     doFlap();
     e.preventDefault();
    }, false);

    document.getElementById('lgd').onkeydown = function (e) {
     if (e.keyCode == 32) {
      doFlap();
      e.preventDefault();
     }
